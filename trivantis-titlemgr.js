...
{
  // LD-7956: Use Math.floor() instead of Math.round() since it gives a more even distribution.
  // round(Math.random()*n) produces numbers between 0 and n inclusive with even distribution
  // for numbers between 0 and n but 0 and n themselves are half as likely.
  // floor(Math.random()*n) produces numbers between 0 and n-1 inclusive with even distribution for all numbers.
...
TTPr.LoadPages = function()
{
  ...
  if( this.iNumberRandom > 0 )
  {
    ...
    // LD-7956: This approach is better than the previous method because:
    // 1. It maintains a separate array of available indices that shrinks as items are picked
    // 2. Avoids having to check/skip already picked items
    // 3. Guarantees no duplicates since each index can only be picked once
    // 4. More efficient since we don't need nested loops to find valid selections
    for( i = 0; i < this.arWork.length; i++ )
    {
      if( this.arPicked.length < i + 1)
        this.arPicked.push( i );
      else
        this.arPicked[i] = i;
    }
    ...
    {
      var sel = get_random(this.arPicked.length);
      var arWorkIndex = this.arPicked[sel];
      this.arPicked.splice(sel, 1);
      this.arRTPages.push(this.arWork[arWorkIndex]);   
    }
  }
  else
  {
...
TSPr.LoadPages = function( arr, rand )
{
  ...
  if( this.iNumberRandom > 0 && rand )
  {
    ...
    {
      var sel = get_random(this.arPicked.length);
      var arPageIndex = this.arPicked[sel];
      this.arPicked.splice(sel, 1);
      arr[sIdx++] = this.arPages[arPageIndex];
    }
...
TQPr.load = function (sid, nl) {
    ...
    this.fbByChoices = getNVInt(nl,'fbbychoices', this.fbByChoices);
    this.arChoiceFbs = getNVArray(nl,'archoicefbs', 'feedback');
